...
export const syncMappings = pgTable("sync_mappings", {
  ...
  // row. Partial (NULL-excluding) to leave bid-board-only rows — which legitimately share neither key —
  // unconstrained. Applied by `db:push` AFTER `db:migrate-sync-mappings-dedupe`; createSyncMapping upserts
  // on the procore_project_id index (falling back to a plain insert until it exists).
...
export const closeoutSurveys = pgTable("closeout_surveys", {
  ...
  ratingOverallExperience: integer("rating_overall_experience"),
  ratingCommunication: integer("rating_communication"),
  ratingSchedule: integer("rating_schedule"),
  ratingQuality: integer("rating_quality"),
  ratingHireAgain: integer("rating_hire_again"),
  ratingReferral: integer("rating_referral"),
