...
export class DatabaseStorage implements IStorage {
  ...
  async enqueueBidboardCallback(data: InsertBidboardCallbackOutbox): Promise<BidboardCallbackOutbox> {
    ...
    // The onConflictDoNothing target is the unique index on rfpApprovalRequestId, so a no-op insert only
    // happens on the request-backed path (a duplicate request id). A voting row (NULL id) never conflicts, so
    // `result` is always defined above for it — this fallback is request-backed only.
    if (data.rfpApprovalRequestId != null) {
