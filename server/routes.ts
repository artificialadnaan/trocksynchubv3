...
export async function registerRoutes(
  ...
  app.get("/api/debug/procore-extraction/:projectId", requireAuth, async (req, res) => {
  ...
  app.get("/api/debug/procore-raw-test", requireAuth, async (req, res) => {
    if (process.env.NODE_ENV === "production") {
      return res.status(404).json({ message: "Not found" });
    }
  ...
  app.get("/api/debug/hubspot-pipelines", requireAuth, async (_req: any, res: any) => {
    if (process.env.NODE_ENV === "production") {
      return res.status(404).json({ message: "Not found" });
    }
    const results: any = {
  ...
  app.post("/api/testing/playwright/workshop/run-script", requireAuth, async (req, res) => {
    try {
      ...
      // Workshop script execution is disabled in production for security
      if (process.env.NODE_ENV === "production") {
        return res.status(403).json({ error: "Workshop script execution is disabled in production" });
      }
