...
`runSyncMappingsReconcile(deps, { commit }): Promise<ReconReport>`. Loads ALL `sync_mappings` (raw SQL over the whole table — NOT `getSyncMappings()`'s 200 cap) + `SELECT procore_id FROM procore_projects`; runs the checker; in `commit` mode, per **error** issue (in a per-issue `try/catch` → `failedWrites`, so one bad write can't abort the scan): writes an `audit_logs status='error'` row FIRST (the 15-min `alertScheduler` emails it), THEN upserts a `manual_review_queue` entry on `(project_number, cycle_id)` with a stable per-ISSUE `cycleId = sync-integrity:<type>:<procoreId>:<sorted mappingIds>` (the colliding id + row-pair → distinct collisions, even on the same pair, never collapse). **No first-detection gate:** persistent unresolved drift intentionally re-alerts each run (it's still broken), while the queue upsert keeps the triage surface de-duped so it never grows. A `procore_projects` cache-load failure degrades to an **empty set** (only the orphan check is skipped; cross-column still runs). `info` issues are counted only. Dry-run (default) writes nothing. Returns counts + per-issue detail; snapshot to `.audit/`.
