...
- **Phase-2 identity check (companion fix in `detectPortfolioIdentityMismatch`):** Phase 2 re-validates the resolved portfolio's NUMBER against the deal/trigger number, which would `quarantine` + throw on a cross-number rebuild (linked portfolio's number intentionally differs). Fix: skip the number/name/hubspot checks **only when** the reached portfolio is the mapping's explicit link (`expectedPortfolioProjectId === actual`) **AND** the caller flagged `portfolioFromExistingLink`. That flag is plumbed: Phase 1 sets `result.portfolioLinkedFromExistingMapping` when the gate skips via `source:"mapping"` → the runner forwards it into Phase 2's `identityContext.portfolioFromExistingLink` → `buildExpectedPortfolioIdentity` carries it into `expected`. A fresh create leaves the flag false — even if the webhook path stamped the mapping with the reached id before validation — so its number/name checks still fire and catch a wrong portfolio.
